...
EDEN_TOKEN_ADDRESS=$(echo "$FORGE_TOKEN_DEPLOY_OUTPUT" | grep -oE "Deployed to: 0x[0-9a-fA-F]{40}" | sed 's/Deployed to: //')
export EDEN_TOKEN_ADDRESS

EDEN_TOKEN_DEPLOY_OUTPUT=$(forge create ./src/EdenTokenLL.sol:EdenTokenLL --rpc-url $EVM_RPC_URL --private-key $ALICE_PRIVATE_KEY --constructor-args 1_000_000_000_000_000_000_000_000 tEDEN tEDN $ALICE_PUB_KEY $ALICE_PUB_KEY 0 $ALICE_PUB_KEY)
EDEN_TOKEN_ADDRESS=$(echo "$FORGE_TOKEN_DEPLOY_OUTPUT" | grep -oE "Deployed to: 0x[0-9a-fA-F]{40}" | sed 's/Deployed to: //')
export EDEN_TOKEN_ADDRESS
...
cast send $EDEN_TOKEN_ADDRESS --rpc-url http://$EVM_RPC_URL --private-key $ALICE_PRIVATE_KEY --gas-limit 65000 "transfer(address,uint256)" "$TEST_WALLET" "1000000000000000000000"
